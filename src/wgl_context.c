...
static int choosePixelFormat(_GLFWwindow* window, const _GLFWctxconfig* ctxconfig, const _GLFWfbconfig* fbconfig)
{
	...
	if (_glfw.wgl.ARB_pixel_format)
	{
		...
		else { if (_glfw.wgl.EXT_colorspace) addAttrib(WGL_COLORSPACE_EXT); }
	}
	else
	...
	for (i = 0; i < nativeCount; i++)
	{
		...
		if (_glfw.wgl.ARB_pixel_format)
		{
			...
			if (findAttribValue(WGL_DOUBLE_BUFFER_ARB)) u->doublebuffer = GLFW_TRUE;
		...
		else
		{
			...
			if (pfd.dwFlags & PFD_DOUBLEBUFFER) u->doublebuffer = GLFW_TRUE;
		}
...
static void makeContextCurrentWGL(_GLFWwindow* window)
{
	if (window)
	{
		if (wglMakeCurrent(window->context.wgl.dc, window->context.wgl.handle)) _glfwPlatformSetTls(
			&_glfw.contextSlot,
			window);
		else
		{
...
GLFWbool _glfwInitWGL(void)
{
	...
	_glfw.wgl.CreateContextAttribsARB = (PFNWGLCREATECONTEXTATTRIBSARBPROC)wglGetProcAddress(
...
// Terminate WGL
//
void _glfwTerminateWGL(void) { if (_glfw.wgl.instance) FreeLibrary(_glfw.wgl.instance); }
...
GLFWbool _glfwCreateContextWGL(_GLFWwindow* window, const _GLFWctxconfig* ctxconfig, const _GLFWfbconfig* fbconfig)
{
	...
	{
		if (!_glfw.wgl.ARB_create_context || !_glfw.wgl.ARB_create_context_profile || !_glfw.wgl.
