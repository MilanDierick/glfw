...
void _glfwInputKey(_GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (key >= 0 && key <= GLFW_KEY_LAST)
	{
		...
		else window->keys[key]                                              = (char)action;
...
GLFWAPI void glfwSetInputMode(GLFWwindow* handle, int mode, int value)
{
	...
	else if (mode == GLFW_LOCK_KEY_MODS) window->lockKeyMods = value ? GLFW_TRUE : GLFW_FALSE;
...
GLFWAPI const char* glfwGetKeyName(int key, int scancode)
{
	...
	{
		if (key != GLFW_KEY_KP_EQUAL && (key < GLFW_KEY_KP_0 || key > GLFW_KEY_KP_ADD) && (key < GLFW_KEY_APOSTROPHE ||
			key > GLFW_KEY_WORLD_2)) return NULL;
...
GLFWAPI GLFWcursor* glfwCreateStandardCursor(int shape)
{
	...
	if (shape != GLFW_ARROW_CURSOR && shape != GLFW_IBEAM_CURSOR && shape != GLFW_CROSSHAIR_CURSOR && shape !=
		GLFW_POINTING_HAND_CURSOR && shape != GLFW_RESIZE_EW_CURSOR && shape != GLFW_RESIZE_NS_CURSOR && shape !=
		GLFW_RESIZE_NWSE_CURSOR && shape != GLFW_RESIZE_NESW_CURSOR && shape != GLFW_RESIZE_ALL_CURSOR && shape !=
