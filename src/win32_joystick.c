...
static BOOL CALLBACK deviceCallback(const DIDEVICEINSTANCE* di, void* user)
{
	...
	if (FAILED(IDirectInputDevice8_EnumObjects(device,
		deviceObjectCallback, &data, DIDFT_AXIS | DIDFT_BUTTON | DIDFT_POV)))
	{
...
void _glfwDetectJoystickConnectionWin32(void)
{
	if (_glfw.win32.xinput.instance)
	{
		...
		for (index = 0; index < XUSER_MAX_COUNT; index++)
		{
			...
			for (jid = 0; jid <= GLFW_JOYSTICK_LAST; jid++)
				if (_glfw.joysticks[jid].present && _glfw.joysticks[jid].win32.device == NULL && _glfw.joysticks[jid].
					win32.index == index) break;
	...
	if (_glfw.win32.dinput8.api)
		if (FAILED(IDirectInput8_EnumDevices(_glfw.win32.dinput8.api,
			DI8DEVCLASS_GAMECTRL, deviceCallback, NULL, DIEDFL_ALLDEVICES)))
		{
...
GLFWbool _glfwPlatformInitJoysticks(void)
{
	if (_glfw.win32.dinput8.instance)
		if (FAILED(DirectInput8Create(GetModuleHandle(NULL),
			DIRECTINPUT_VERSION, &IID_IDirectInput8W, (void**) &_glfw.win32.dinput8.api, NULL)))
		{
