...
void _glfwPlatformSetWindowIcon(_GLFWwindow* window, int count, const GLFWimage* images)
{
	...
	if (count)
	{
		...
		const GLFWimage* smallImage = chooseImage(count,
												  images,
...
void _glfwPlatformSetWindowSizeLimits(_GLFWwindow* window, int minwidth, int minheight, int maxwidth, int maxheight)
{
	...
	if ((minwidth == GLFW_DONT_CARE || minheight == GLFW_DONT_CARE) && (maxwidth == GLFW_DONT_CARE || maxheight ==
		GLFW_DONT_CARE)) return;
...
float _glfwPlatformGetWindowOpacity(_GLFWwindow* window)
{
	...
	if ((GetWindowLongW(window->win32.handle, GWL_EXSTYLE) & WS_EX_LAYERED) && GetLayeredWindowAttributes(
		window->win32.handle,
		NULL,
		&alpha,
		&flags))
...
{
	if (mode == GLFW_CURSOR_DISABLED) { if (_glfwPlatformWindowFocused(window)) disableCursor(window); }
	else if (_glfw.win32.disabledCursorWindow == window) enableCursor(window);
	else if (cursorInContentArea(window)) updateCursorImage(window);
}
...
int _glfwPlatformCreateStandardCursor(_GLFWcursor* cursor, int shape)
{
	...
	if (shape == GLFW_ARROW_CURSOR) id = OCR_NORMAL;
	else if (shape == GLFW_IBEAM_CURSOR) id = OCR_IBEAM;
	else if (shape == GLFW_CROSSHAIR_CURSOR) id = OCR_CROSS;
	else if (shape == GLFW_POINTING_HAND_CURSOR) id = OCR_HAND;
	else if (shape == GLFW_RESIZE_EW_CURSOR) id = OCR_SIZEWE;
	else if (shape == GLFW_RESIZE_NS_CURSOR) id = OCR_SIZENS;
	else if (shape == GLFW_RESIZE_NWSE_CURSOR) id = OCR_SIZENWSE;
	else if (shape == GLFW_RESIZE_NESW_CURSOR) id = OCR_SIZENESW;
	else if (shape == GLFW_RESIZE_ALL_CURSOR) id = OCR_SIZEALL;
	else if (shape == GLFW_NOT_ALLOWED_CURSOR) id = OCR_NO;
	else
	{
...
EGLenum _glfwPlatformGetEGLPlatform(EGLint** attribs)
{
	if (_glfw.egl.ANGLE_platform_angle)
	{
		...
		{
			if (_glfw.hints.init.angleType == GLFW_ANGLE_PLATFORM_TYPE_OPENGL) type =
				EGL_PLATFORM_ANGLE_TYPE_OPENGL_ANGLE;
			else if (_glfw.hints.init.angleType == GLFW_ANGLE_PLATFORM_TYPE_OPENGLES) type =
		...
		if (_glfw.egl.ANGLE_platform_angle_d3d)
		{
			...
			else if (_glfw.hints.init.angleType == GLFW_ANGLE_PLATFORM_TYPE_D3D11) type =
				EGL_PLATFORM_ANGLE_TYPE_D3D11_ANGLE;
		}

		if (_glfw.egl.ANGLE_platform_angle_vulkan)
			if (_glfw.hints.init.angleType == GLFW_ANGLE_PLATFORM_TYPE_VULKAN) type =
