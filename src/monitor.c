...
_GLFWmonitor* _glfwAllocMonitor(const char* name, int widthMM, int heightMM)
{
	...
	if (name) monitor->name = _glfw_strdup(name);
...
const GLFWvidmode* _glfwChooseVideoMode(_GLFWmonitor* monitor, const GLFWvidmode* desired)
{
	...
	for (i = 0; i < monitor->modeCount; i++)
	{
		...
		sizeDiff = abs(
			(current->width - desired->width) * (current->width - desired->width) + (current->height - desired->height)
		...
		if ((colorDiff < leastColorDiff) || (colorDiff == leastColorDiff && sizeDiff < leastSizeDiff) || (colorDiff ==
			leastColorDiff && sizeDiff == leastSizeDiff && rateDiff < leastRateDiff))
		{
			closest        = current;
